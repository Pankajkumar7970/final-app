...
const CompareScreen = ({ navigation }) => {
  ...
  const renderLineChart = () => {
    ...
    datasets.push({
      data: months
        .filter(
          (_, i) => i % Math.max(1, Math.floor(selectedHorizon / 6)) === 0
        )
    ...
    selectedInstruments.forEach((instrument, index) => {
      ...
      datasets.push({
        data: months
          ...
          .map((month) =>
            calculateCompoundInterest(0, contribution, rate, month)
          ),
    ...
    const chartData = {
      ...
      legend: [
        "Savings",
        ...selectedInstruments.map((key) => investmentOptions[key].name),
      ],
    };
  ...
  const renderBarChart = () => {
    ...
    const labels = [
      ...
      ...selectedInstruments.map(
        (key) => investmentOptions[key].name.split(" ")[0]
      ),
    ];
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <BarChart
            ...
            chartConfig={{
              ...
              backgroundGradientTo: "#ffffff",
              decimalPlaces: 0,
              color: (opacity = 1) => `${PSBColors.primary.green}`,
              labelColor: (opacity = 1) => `rgba(51, 65, 85, ${opacity})`,
              style: { borderRadius: 16 },
            }}
            style={styles.chart}
  ...
  const renderComparisonTable = () => {
    ...
    const tableData = selectedInstruments
      .map((instrument) => {
