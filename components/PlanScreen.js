...
const PlanScreen = ({ navigation }) => {
    ...
    try {
      ...
      const [
        ...
        api.getFinancialHealthScore(
          ...
          surplus,
          numericGoal,
          numericCurrentSavings
        ),
        api.getWealthGapAnalysis(
          numericAge,
          numericSalary * 12,
          numericCurrentSavings
        ),
        api.calculateFIRE(
          Object.values(numericExpenses).reduce((sum, val) => sum + val, 0),
          numericCurrentSavings,
          numericSalary
        ),
      ]);
      ...
      Alert.alert(
        "Error",
        "Failed to calculate projections. Please check your connection."
      );
    } finally {
      setLoading(false);
    }
    ...
    <View style={styles.inputSection}>
      ...
      <TouchableOpacity
        style={[
          ...
          surplus <= 0 && styles.calculateButtonDisabled,
        ]}
            ...
            {recommendations.portfolioSuggestion && (
              <View style={styles.portfolioSuggestionCard}>
                ...
                <View style={styles.allocationContainer}>
                  {Object.entries(
                    recommendations.portfolioSuggestion.allocation
                  ).map(([key, value], index) => (
  ...
  const getInvestmentRecommendation = (option, goalMonths) => {
    ...
    if (goalYears <= 3) {
      if (["Fixed Deposit", "Recurring Deposit"].includes(instrument)) {
        ...
        recommendation.warnings.push(
          "⚠️ High volatility risk for short-term goals"
        );
      ...
      if (["Mutual Funds", "Gold ETF"].includes(instrument)) {
        recommendation.reasons.push(
          "✅ Good for medium-term goals (3-7 years)"
        );
        recommendation.priority = "high";
      }
    } else {
      if (
        ["PPF", "Nifty ETF", "Mutual Funds", "Sovereign Gold Bond"].includes(
          instrument
        )
      ) {
        recommendation.reasons.push(
          "✅ Excellent for long-term goals (≥7 years)"
        );
        recommendation.priority = "high";
      }
    }
    ...
    if (["PPF", "Fixed Deposit", "Recurring Deposit"].includes(instrument)) {
      ...
      recommendation.reasons.push(
        "✅ Additional 2.5% interest + gold appreciation"
      );
    } else if (["Mutual Funds", "Nifty ETF"].includes(instrument)) {
    ...
    if (instrument === "PPF") {
      recommendation.reasons.push(
        "✅ 80C deduction + tax-free growth & withdrawal"
      );
  ...
  const renderInvestmentOptions = () => {
    ...
    const goalMonths =
      parseFloat(goal) / (calculatedData?.savingsPerMonth || 1);
            ...
            return (
              <View
                ...
                style={[
                  ...
                  recommendation.priority === "high" &&
                    styles.investmentCardRecommended,
  ...
  const renderChart = () => {
    ...
    const savingsLine = months.map(
      (month) => calculatedData.savingsPerMonth * month
    );
  ...
  const renderCurrentRates = () => (
    <View style={styles.ratesSection}>
      <View style={styles.ratesHeader}>
        ...
        <TouchableOpacity
          style={styles.editRatesButton}
          onPress={handleEditRates}
        >
              ...
              <View key={instrument} style={styles.rateInputItem}>
                ...
                <View style={styles.rateInputContainer}>
                  <TextInput
                    ...
                    value={rate.toString()}
                    onChangeText={(value) =>
                      handleRateChange(instrument, value)
                    }
                    keyboardType="numeric"
                    placeholder="0.0"
                  />
