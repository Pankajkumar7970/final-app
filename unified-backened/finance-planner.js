...
function generatePersonalizedRecommendations(
  ...
  if (savingsRate < 20) {
    recommendations.push({
      type: "savings_rate",
      message:
        "Since your savings rate is only " +
        savingsRate.toFixed(1) +
        "%, you should improve savings before going into Mutual Funds.",
      priority: "high",
    });
    ...
    recommendations.push({
      ...
      message:
        ...
        savingsRate.toFixed(1) +
        "% is good, but aim for 30% to accelerate your financial goals.",
      priority: "medium",
    });
  } else {
    recommendations.push({
      type: "savings_rate",
      message:
        "Excellent savings rate of " +
        savingsRate.toFixed(1) +
        "%! You are on track to achieve your financial goals faster.",
      priority: "low",
    });
  }
  ...
  if (rentPercentage > 30) {
    recommendations.push({
      ...
      message:
        "Your housing cost is " +
        rentPercentage.toFixed(1) +
        "% of income, which is high. The recommended is below 30%.",
      priority: "high",
    });
  }
...
function getScoreMessage(category, score) {
  ...
  const scoreKeys = Object.keys(categoryMessages)
    .map(Number)
    .sort((a, b) => a - b);
...
function analyzeWealthGap(age, annualIncome, currentSavings) {
  ...
  if (wealthGap >= 0) {
    result.message = `You're ahead of schedule by ₹${Math.abs(
      wealthGap
    ).toLocaleString()}! Keep up the good work.`;
  } else {
    result.message = `You're behind schedule by ₹${Math.abs(
      wealthGap
    ).toLocaleString()}. Consider increasing your savings rate.`;
  }
...
function calculateFIRE(
  ...
  if (currentSavings >= fireNumber) {
    ...
    const numerator = Math.log(
      (annualSavings * (1 + r)) /
        (annualSavings + (fireNumber - currentSavings) * r)
    );
  ...
  if (yearsToFIRE === 0) {
    result.message =
      "Congratulations! You have already achieved financial independence!";
  } else if (yearsToFIRE === Infinity) {
    result.message =
      "With your current savings rate, FIRE is not achievable. Try increasing your savings or reducing expenses.";
...
function suggestPortfolioSplit(goalTimeframeYears) {
  ...
  if (goalTimeframeYears < 2) {
    ...
    portfolioSuggestion = {
      type: "short_term",
      message:
        "For goals less than 2 years away, focus on RD/FD to preserve capital.",
      allocation: {
        fd: 70,
        rd: 30,
      },
      explanation:
        "Short-term goals require capital preservation. Fixed and Recurring Deposits offer guaranteed returns without market risk.",
    };
    ...
    portfolioSuggestion = {
      type: "medium_term",
      message:
        "For goals 3-5 years away, consider a mix of Debt and Balanced Mutual Funds.",
      allocation: {
        debt_funds: 60,
        balanced_funds: 30,
        fd: 10,
      ...
      explanation:
        "Medium-term goals allow for some market exposure while maintaining stability. A mix of debt and balanced funds provides moderate growth with managed risk.",
    };
  } else {
    ...
    portfolioSuggestion = {
      type: "long_term",
      message:
        "For goals 10+ years away, focus on Mutual Funds/Nifty 50 with some Gold allocation.",
      allocation: {
        equity_funds: 70,
        nifty_etf: 20,
        gold: 10,
      ...
      explanation:
        "Long-term goals benefit from equity exposure. The stock market historically outperforms other asset classes over 10+ year periods, while gold provides diversification.",
    };
  }
...
function generateExampleTexts(savingsPerMonth, goal) {
  ...
  examples.push({
    type: "comparison",
    message: `Your money in FD gives ₹${(fdAmount / 1000).toFixed(
      1
    )}L in ${fdYears} years, but Mutual Funds could give ₹${(
      mfAmount / 1000
    ).toFixed(1)}L – a difference of ₹${(difference / 1000).toFixed(
      1
    )}K, enough for a significant purchase.`,
  });
  ...
  examples.push({
    type: "inflation",
    message: `Inflation will eat ${inflationImpact.toFixed(
      0
    )}% of your FD return, so real profit is only ${realFdReturn.toFixed(1)}%.`,
  });
...
module.exports = function (app) {
  ...
  app.post("/api/personalized-recommendations", (req, res) => {
    try {
      const { income, expenses, savingsPerMonth, goal, goalTimeframeYears } =
        req.body;

      if (!income || !expenses || !savingsPerMonth || !goal) {
        return res.status(400).json({
          error:
            "Missing required fields: income, expenses, savingsPerMonth, goal",
        });
      }
      ...
      const portfolioSuggestion = suggestPortfolioSplit(
        goalTimeframeYears || 5
  ...
  app.post("/api/financial-health-score", (req, res) => {
    try {
      const { income, expenses, savingsPerMonth, goal, currentSavings } =
        req.body;
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Missing required fields: income, expenses, savingsPerMonth, goal, currentSavings",
        });
      }
  ...
  // POST /wealth-gap
  app.post("/api/wealth-gap", (req, res) => {
    try {
      ...
      // Analyze wealth gap
      const wealthGapAnalysis = analyzeWealthGap(
        age,
        annualIncome,
        currentSavings
      );
  ...
  app.post("/api/fire-calculator", (req, res) => {
    try {
      const {
        monthlyExpenses,
        currentSavings,
      ...
      if (!monthlyExpenses || currentSavings === undefined || !monthlyIncome) {
        return res.status(400).json({
          error:
            "Missing required fields: monthlyExpenses, currentSavings, monthlyIncome",
        });
      }
    ...
    try {
      ...
      if (savingsPerMonth <= 0) {
        return res.json({
          ...
          timeToGoalDays: -1,
          message:
            "No surplus available for savings. Consider reducing expenses.",
        });
      }
  ...
  app.post("/api/calculate-investments", (req, res) => {
    try {
      ...
      const savingsOnly = {
        ...
        finalAmount: goal,
        totalContribution:
          savingsPerMonth * calculateTimeToGoal(savingsPerMonth, goal, 0),
        gain: 0,
      };
      ...
      Object.keys(INVESTMENT_OPTIONS).forEach((key) => {
        ...
        results[key] = {
          ...
          gainPercentage:
            totalContribution > 0
              ? Math.round((gain / totalContribution) * 100)
              : 0,
        };
      });
        ...
        Object.keys(INVESTMENT_OPTIONS).forEach((key) => {
          ...
          const rate =
            customRates && customRates[key]
              ? customRates[key]
              : option.defaultRate;
          projections[months][key] = calculateCompoundInterest(
            0,
            savingsPerMonth,
            rate,
            months
      ...
      res.json({
        ...
        comparisonVsSavings: Object.keys(results)
          ...
          .map((key) => ({
            ...results[key],
            timeSaved:
              savingsOnly.timeToGoalMonths - results[key].timeToGoalMonths,
            timeSavedPercentage: Math.round(
              ((savingsOnly.timeToGoalMonths - results[key].timeToGoalMonths) /
                savingsOnly.timeToGoalMonths) *
                100
            ),
          }))
  ...
  app.post("/api/improvement-tips", (req, res) => {
    try {
      ...
      const currentTimeToGoal =
        currentSavings > 0
          ? calculateTimeToGoal(currentSavings, savingsGoal)
          : -1;
        ...
        if (amount > 0) {
          ...
          reductions.forEach((reduction) => {
            if (reduction < amount && reduction > 0) {
              ...
              const newTimeToGoal = calculateTimeToGoal(
                newSavings,
                savingsGoal
              );
              ...
              if (timeSaved > 0) {
                tips.push({
                  ...
                  impact:
                    timeSaved > 6 ? "high" : timeSaved > 2 ? "medium" : "low",
                });
              }
            }
          });
      ...
      // Investment tips
      if (currentSavings > 0) {
        const bestInvestmentOption = Object.keys(INVESTMENT_OPTIONS).reduce(
          (best, current) => {
            const currentTime = calculateTimeToGoal(
              currentSavings,
              savingsGoal,
              INVESTMENT_OPTIONS[current].defaultRate
            );
            const bestTime = calculateTimeToGoal(
              currentSavings,
              savingsGoal,
              INVESTMENT_OPTIONS[best].defaultRate
            );
            return currentTime < bestTime ? current : best;
          }
        );

        const investmentTimeToGoal = calculateTimeToGoal(
          currentSavings,
          savingsGoal,
          INVESTMENT_OPTIONS[bestInvestmentOption].defaultRate
        );
      ...
      const learningContent = [
        {
          ...
          category: "Basics",
          summary:
            "Key factors to evaluate before making any investment decision",
          content:
            "Before investing, check: 1) Expense ratio (lower is better), 2) Track record (consistent performance), 3) Benchmark comparison, 4) Your risk profile, 5) Tax implications, 6) Lock-in period, 7) Liquidity needs",
          learnMore:
            "Expense ratio should be <1% for equity funds, <0.5% for debt funds. Check 5-year returns, not just 1-year. Match investment risk with your risk appetite. Consider your investment horizon - don't invest in equity if you need money in <5 years.",
        },
        {
          id: 2,
          title: "PPF Basics",
          category: "Tax-saving",
          summary:
            "Understanding Public Provident Fund benefits and limitations",
          content:
            "PPF offers 7.1% tax-free returns with 15-year lock-in. Max investment ₹1.5L/year. Triple tax benefit - deduction, tax-free growth, tax-free withdrawal.",
          learnMore:
            "PPF is ideal for long-term goals like retirement. You can extend in 5-year blocks after 15 years. Partial withdrawals allowed after 7th year. No risk as it's government-backed.",
        },
        {
          id: 3,
          title: "What is SIP?",
          ...
          content:
            "SIP helps invest fixed amount regularly in mutual funds. Benefits: rupee cost averaging, disciplined investing, power of compounding.",
          learnMore:
            "Start SIP as early as possible. Increase SIP amount annually by 10-15%. Don't stop SIP during market downturns - that's when you buy more units at lower prices.",
        },
        {
          id: 4,
          ...
          content:
            "Gold ETF: traded like stocks, no lock-in, 3% expense ratio. SGB: 8-year lock-in, 2.5% additional interest, tax-free if held till maturity.",
          learnMore:
            "SGB is better for long-term holders due to tax benefits and additional interest. Gold ETF is better for tactical allocation with flexibility to exit anytime.",
        },
        {
          id: 5,
          ...
          content:
            "FD: guaranteed returns, fully taxable. Debt MF: potentially higher returns, tax-efficient after 3 years, some risk.",
          learnMore:
            "For >3 years horizon, debt funds can be more tax-efficient. For emergency funds, FD/liquid funds are better. Ultra short-term funds for 6 months to 2 years.",
        },
        {
          id: 6,
          ...
          content:
            "Higher potential returns come with higher risk. Equity > Debt > FD in both risk and potential returns.",
          learnMore:
            "Don't chase high returns without understanding risks. Diversify across asset classes. Your risk capacity depends on age, income stability, and investment horizon.",
        },
        {
          id: 7,
          ...
          learnMore:
            "Asset allocation example: 60% equity, 30% debt, 10% gold for young investors. Rebalance annually. International diversification through global funds.",
        },
        {
          id: 8,
          ...
          content:
            "Check: tracking error (lower better), expense ratio (<0.5%), fund size (>₹500 cr), tracking difference.",
          learnMore:
            "Nifty 50 for large-cap exposure, Nifty Next 50 for mid-cap, Nifty 500 for broader market. Choose funds with consistent low tracking error.",
        },
        {
          id: 9,
          ...
          content:
            "RD allows monthly fixed deposits with guaranteed returns. Good for risk-averse investors wanting disciplined savings.",
          learnMore:
            "RD is like SIP for bank deposits. Returns are lower than equity but guaranteed. Good for short-term goals (1-5 years) where capital protection is important.",
        },
        {
          id: 10,
          ...
          content:
            "LTCG on equity: 10% above ₹1L. STCG on equity: 15%. Debt funds taxed as per income slab after 2023 budget changes.",
          learnMore:
            "Hold equity investments for >1 year to get LTCG benefit. Debt funds lost indexation benefit post-2023. Consider tax-saving instruments like ELSS, PPF, ULIP for 80C deduction.",
        },
      ];
